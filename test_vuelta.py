...
class TestTourOfSpain:
    ...
    def test_vuelta1935(self):
        route="""1,29 April,Madrid to Valladolid,185.0 km,Road stage
        ...
        edition = vuelta.vuelta1935()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "14 stages"

    def test_vuelta1936(self):
        route="""1,5 May,Madrid to Salamanca,210.0 km,Road stage
        ...
        edition = vuelta.vuelta1936()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta1941(self):
        route="""1,12 June,Madrid to Salamanca,210.0 km,Road stage
        ...
        edition = vuelta.vuelta1941()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages including 1 split stage"

    def test_vuelta1942(self):
        route="""1,30 June,Madrid to Albacete,245.0 km,Road stage
        ...
        edition = vuelta.vuelta1942()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages including 1 split stage"

    def test_vuelta1945(self):
        route="""1,10 May,Madrid to Salamanca,212.0 km,Road stage
        ...
        edition = vuelta.vuelta1945()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "18 stages including 1 split stage"

    def test_vuelta1946(self):
        route="""1,7 May,Madrid to Salamanca,212.0 km,Road stage
        ...
        edition = vuelta.vuelta1946()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "20 stages including 3 split stages"

    def test_vuelta1947(self):
        route="""1,12 May,Madrid to Albacete,243.0 km,Road stage
        ...
        edition = vuelta.vuelta1947()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "24 stages including 1 split stage"

    def test_vuelta1948(self):
        route="""1a,13 June,Madrid,14.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1948()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages including 1 split stage"

    def test_vuelta1950(self):
        route="""1,17 August,Madrid to Valladolid,190.0 km,Road stage
        ...
        edition = vuelta.vuelta1950()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages including 3 split stages"

    def test_vuelta1955(self):
        route="""1,23 April,Bilbao to San Sebastián,240.0 km,Road stage
        ...
        edition = vuelta.vuelta1955()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "15 stages"

    def test_vuelta1956(self):
        route="""1,26 April,Bilbao to Santander,203.0 km,Road stage
        ...
        edition = vuelta.vuelta1956()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages including 2 split stages"

    def test_vuelta1957(self):
        route="""1,26 April,Bilbao to Vitoria,158.0 km,Road stage
        ...
        edition = vuelta.vuelta1957()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "16 stages"

    def test_vuelta1958(self):
        route="""1,30 April,Bilbao to San Sebastián,164.0 km,Road stage
        ...
        edition = vuelta.vuelta1958()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "16 stages including 2 split stages"

    def test_vuelta1959(self):
        route="""1a,29 April,Madrid,9.0 km,Team time trial
        ...
        edition = vuelta.vuelta1959()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages including 1 split stage"

    def test_vuelta1960(self):
        route="""1,29 April,Gijón,8.0 km,Team time trial
        ...
        edition = vuelta.vuelta1960()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages including 1 split stage"

    def test_vuelta1961(self):
        route="""1a,26 April,San Sebastián,10.5 km,Team time trial
        ...
        edition = vuelta.vuelta1961()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "16 stages including 1 split stage"

    def test_vuelta1962(self):
        route="""1,27 April,Barcelona,90.0 km,Road stage
        ...
        edition = vuelta.vuelta1962()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages"

    def test_vuelta1963(self):
        route="""1a,1 May,Gijón to Mieres,45.0 km,Road stage
        ...
        edition = vuelta.vuelta1963()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "15 stages including 2 split stages"

    def test_vuelta1964(self):
        route="""1a,30 April,Benidorm to Barcelona,42.0 km,Road stage
        ...
        edition = vuelta.vuelta1964()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages including 2 split stages"

    def test_vuelta1965(self):
        route="""1,29 April,Vigo,168.0 km,Road stage
        ...
        edition = vuelta.vuelta1965()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "18 stages including 2 split stages"

    def test_vuelta1966(self):
        route="""1a,28 April,Murcia,111.0 km,Road stage
        ...
        edition = vuelta.vuelta1966()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "18 stages including 4 split stages"

    def test_vuelta1967(self):
        route="""1a,27 April,Vigo to O Baixo Miño,110.0 km,Road stage
        ...
        edition = vuelta.vuelta1967()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "18 stages including 3 split stages"

    def test_vuelta1968(self):
        route="""1a,25 April,Zaragoza,130.0 km,Road stage
        ...
        edition = vuelta.vuelta1968()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "18 stages including 2 split stages"

    def test_vuelta1969(self):
        route="""1A,23 April,Badajoz,6.5 km,Individual time trial
        ...
        edition = vuelta.vuelta1969()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages including 2 split stages"

    def test_vuelta1970(self):
        route="""P,23 April,Cádiz,6.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1970()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 2 split stages"

    def test_vuelta1971(self):
        route="""P,29 April,Almería,4.2 km,Individual time trial
        ...
        edition = vuelta.vuelta1971()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages + Prologue including 2 split stages"

    def test_vuelta1972(self):
        route="""P,27 April,Fuengirola,6.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1972()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages + Prologue including 3 split stages"

    def test_vuelta1973(self):
        route="""P,26 April,Calp,5.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1973()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "17 stages + Prologue including 4 split stages"

    def test_vuelta1974(self):
        route="""P,23 April,Almería,5.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1974()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 3 split stages"

    def test_vuelta1975(self):
        route="""P,22 April,Fuengirola,4.4 km,Individual time trial
        ...
        edition = vuelta.vuelta1975()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 1 split stage"

    def test_vuelta1976(self):
        route="""P,27 April,Estepona,3.2 km,Individual time trial
        ...
        edition = vuelta.vuelta1976()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "20 stages + Prologue including 1 split stage"

    def test_vuelta1977(self):
        route="""P,26 April,Dehesa de Campoamor,8.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1977()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 1 split stage"

    def test_vuelta1978(self):
        route="""P,25 April,Gijón,8.6 km,Individual time trial
        ...
        edition = vuelta.vuelta1978()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 2 split stages"

    def test_vuelta1979(self):
        route="""P,24 April,Jerez de la Frontera,6.3 km,Individual time trial
        ...
        edition = vuelta.vuelta1979()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 3 split stages"

    def test_vuelta1980(self):
        route="""P,22 April,La Manga,10.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1980()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 1 split stage"

    def test_vuelta1981(self):
        route="""P,21 April,Santander,6.3 km,Individual time trial
        ...
        edition = vuelta.vuelta1981()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 2 split stages"

    def test_vuelta1982(self):
        route="""P,20 April,Santiago de Compostela,6.7 km,Individual time trial
        ...
        edition = vuelta.vuelta1982()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 2 split stages"

    def test_vuelta1983(self):
        route="""P,19 April,Almussafes,6.8 km,Individual time trial
        ...
        edition = vuelta.vuelta1983()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 1 split stage"

    def test_vuelta1984(self):
        route="""P,17 April,Jerez de la Frontera,6.6 km,Individual time trial
        ...
        edition = vuelta.vuelta1984()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue including 1 split stage"

    def test_vuelta1985(self):
        route="""P,23 April,Valladolid,5.6 km,Individual time trial
        ...
        edition = vuelta.vuelta1985()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "19 stages + Prologue"

    def test_vuelta1986(self):
        route="""P,22 April,Palma de Mallorca,5.7 km,Individual time trial
        ...
        edition = vuelta.vuelta1986()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages + Prologue"

    def test_vuelta1987(self):
        route="""P,23 April,Benidorm,6.6 km,Individual time trial
        ...
        edition = vuelta.vuelta1987()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages + Prologue"

    def test_vuelta1988(self):
        route="""1,25 April,Santa Cruz de Tenerife,17.4 km,Individual time trial
        ...
        edition = vuelta.vuelta1988()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta1989(self):
        route="""1,24 April,A Coruña,21.0 km,Road stage
        ...
        edition = vuelta.vuelta1989()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages including 1 split stage"

    def test_vuelta1990(self):
        route="""1,24 April,Benicàssim,10.3 km,Individual time trial
        ...
        edition = vuelta.vuelta1990()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages including 1 split stage"

    def test_vuelta1991(self):
    ...
    def test_vuelta1992(self):
        route="""1,27 April,Jerez de la Frontera,9.2 km,Individual time trial
        ...
        edition = vuelta.vuelta1992()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages including 1 split stage"

    def test_vuelta1993(self):
        route="""1,26 April,A Coruña,10.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1993()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta1994(self):
        route="""1,25 April,Valladolid,9.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1994()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta1995(self):
        route="""P,2 September,Zaragoza,7.0 km,Individual time trial
        ...
        edition = vuelta.vuelta1995()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages + Prologue"

    def test_vuelta1996(self):
        route="""1,7 September,Valencia,162.0 km,Road stage
        ...
        edition = vuelta.vuelta1996()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages"

    def test_vuelta1997(self):
        route="""1,6 September,Lisbon (Portugal) to Estoril (Portugal),155.0 km,Road stage
        ...
        edition = vuelta.vuelta1997()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages"

    def test_vuelta1998(self):
        route="""1,5 September,Córdoba,161.7 km,Road stage
        ...
        edition = vuelta.vuelta1998()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "22 stages"

    def test_vuelta1999(self):
        route="""P,4 September,Murcia,6.1 km,Individual time trial
        ...
        edition = vuelta.vuelta1999()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages + Prologue"

    def test_vuelta2000(self):
        route="""1,26 August,Málaga,13.3 km,Individual time trial
        ...
        edition = vuelta.vuelta2000()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2001(self):
        route="""1,8 September,Salamanca,12.0 km,Individual time trial
        ...
        edition = vuelta.vuelta2001()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2002(self):
        route="""1,7 September,Valencia,24.6 km,Team time trial
        ...
        edition = vuelta.vuelta2002()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2003(self):
        route="""1,6 September,Gijón,28.0 km,Team time trial
        ...
        edition = vuelta.vuelta2003()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2004(self):
        route="""1,4 September,León,28.0 km,Team time trial
    ...
    def test_vuelta2005(self):
        route="""1,27 August,Granada,7.0 km,Individual time trial
        ...
        edition = vuelta.vuelta2005()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2006(self):
        route="""1,26 August,Málaga,7.2 km,Team time trial
        ...
        edition = vuelta.vuelta2006()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2007(self):
        route="""1,1 September,Vigo,146.4 km,Road stage
        ...
        edition = vuelta.vuelta2007()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2008(self):
        route="""1,30 August,Granada,7.7 km,Individual time trial
    ...
    def test_vuelta2009(self):
        route="""1,29 August,Assen (Netherlands),4.8 km,Individual time trial
        ...
        edition = vuelta.vuelta2009()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2010(self):
        route="""1,28 August,Seville,13.0 km,Team time trial
        ...
        edition = vuelta.vuelta2010()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2011(self):
        route="""1,20 August,Benidorm,13.5 km,Team time trial
        ...
        edition = vuelta.vuelta2011()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2012(self):
        route="""1,18 August,Pamplona,16.5 km,Team time trial
        ...
        edition = vuelta.vuelta2012()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2013(self):
        route="""1,24 August,Vilanova de Arousa to Sanxenxo,27.4 km,Team time trial
        ...
        edition = vuelta.vuelta2013()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2014(self):
        route="""1,23 August,Jerez de la Frontera,12.6 km,Team time trial
        ...
        edition = vuelta.vuelta2014()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2015(self):
        route="""1,22 August,Puerto Banus to Marbella,7.4 km,Team time trial
        ...
        edition = vuelta.vuelta2015()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2016(self):
        route="""1,20 August,Laias to Parque Náutico Castrelo de Miño,7.4 km,Team time trial
        ...
        edition = vuelta.vuelta2016()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2017(self):
        route="""1,19 August,Nîmes (France),13.7 km,Team time trial
        ...
        edition = vuelta.vuelta2017()
        assert edition.route == inspect.cleandoc(route)
        assert edition.stage_summary == "21 stages"

    def test_vuelta2018(self):
        route="""1,25 August,Málaga,8.0 km,Individual time trial
